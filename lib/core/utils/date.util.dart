...
dynamic getPreviousMonthDate(dynamic date) {
  ...
  return date.month == 1
      ? DateTime((date as DateTime).year - 1, 12, 1)
      : DateTime((date as DateTime).year, date.month - 1, 1);
}
...
dynamic getNextMonthDate(dynamic date) {
  ...
  return date.month == 12
      ? DateTime((date as DateTime).year + 1, 1, 1)
      : DateTime((date as DateTime).year, date.month + 1, 1);
}
...
dynamic getValidDate(dynamic minDate, dynamic maxDate, dynamic date) {
  if ((date as DateTime).isAfter((minDate as DateTime))) {
    if ((date as DateTime).isBefore((maxDate as DateTime))) {
      return date;
    } else {
      return maxDate;
    }
...
bool isDateWithInDateRange(dynamic startDate, dynamic endDate, dynamic date) {
  ...
  if ((startDate as DateTime).isAfter((endDate as DateTime))) {
...
bool isSameOrBeforeDate(dynamic lastDate, dynamic date) {
  return isSameDate(lastDate, date) || (lastDate as DateTime).isAfter((date as DateTime));
}

/// Check the date after/same of first date
bool isSameOrAfterDate(dynamic firstDate, dynamic date) {
  return isSameDate(firstDate, date) || (firstDate as DateTime).isBefore((date as DateTime));
}
...
dynamic addDays(dynamic date, int days) {
  ...
  return DateTime((date as DateTime).year, date.month, date.day + days);
}
...
dynamic getFirstDayOfWeekDate(
  ...
  if (visibleDatesCount == 42) {
    if (currentDate is HijriDateTime) {
      ...
      currentDate = DateTime((currentDate as DateTime).year, (currentDate).month, 1);
    }
  }

  int value = -(currentDate as DateTime).weekday + firstDayOfWeek - numberOfWeekDays;
